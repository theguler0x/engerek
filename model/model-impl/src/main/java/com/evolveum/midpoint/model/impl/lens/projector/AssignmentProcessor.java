			...
			for (ExclusionPolicyConstraintType exclusionA : policyConstraints.getExclusion()) {
				...
				if (roleB.getOid().equals(targetRef.getOid())) {
					if (exclusionA.getEnforcement() == null || exclusionA.getEnforcement() == PolicyConstraintEnforcementType.ENFORCE) {
						/* throw new PolicyViolationException("Görevler ayrılığı politikası ihlali: "+roleA.getRole()+" ile "+roleB.getRole()+
								" ,aynı anda atanamaz"); */
						throw new PolicyViolationException("Görevler ayrılığı politikası ihlali: Bu iki rol aynı anda atanamaz");
					} else {
